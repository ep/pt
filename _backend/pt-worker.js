/*
  ...
  SETTINGS (Worker > Settings > Variables and Secrets)
    GATED_TOOLS   Variable. Optional. Comma-separated tool ids that require a
                  ...
                  type anything.
    ALLOW_ORIGIN  Variable. Comma-separated origins allowed to call this worker,
                  e.g. "https://ep.github.io". Leave unset while setting up.
...
export default {
  async fetch(request, env) {
    ...
    var allowList = (env.ALLOW_ORIGIN || '').split(',').map(function (s) { return s.trim(); }).filter(Boolean);
    var allowOrigin = '*';
    if (allowList.length) {
