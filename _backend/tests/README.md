...
- `qa-pair-poll.mjs` tests `../../field-tools/pair-poll/index.html` (Pair Poll) in a
  ...
  fake database, so the console and two phones talk to the actual gate and
  prefix-read logic. It covers the studio, setup codes, the bot test drive in both
  reveal cadences, named and anonymous lobbies, voting and changing your mind,
  mirrored reveals, recovery after reload, the recap and report, coachmarks, mute,
  and the gate screens.
